...
- Lines that start with `/` are **local netplay/debug commands** routed through `netConsoleCommand` instead of being broadcast as chat. They work even outside a net session so you can pre-configure things like lag before connecting. Available commands:
  ...
  - `/wireframe [on|off]` (alias `/wf`) — toggle the Wireframe cheat (`CHEAT_WIREFRAME`) live, no stage reload. Flips the cheat's active+enabled bits in `g_CheatsActiveBank1`/`g_CheatsEnabledBank1`; `bgTickPortals` pushes the state into the renderer's `gfx_wireframe_mode` each in-game frame. No arg toggles. Unrelated to netplay but lives here because all `/`-commands route through `netConsoleCommand`. See `docs/PORT_WIREFRAME.md`.
  - `/help` / `/?` — list commands.
