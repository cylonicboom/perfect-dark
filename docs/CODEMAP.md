...
| `video.c` | SDL3 window creation, fast3d renderer integration; selects the OpenGL (default) or SDL_GPU backend via `Video.Renderer`/`--renderer` |
| `input.c` | SDL3 keyboard/mouse/gamepad input, virtual-key bindings, controller remapping |
| `audio.c` | SDL3 audio backend: feeds N64 audio-manager output to the hardware audio stream |
...
| `gfx_sdl.h` | SDL3 window manager for fast3d (implemented in `gfx_sdl.cpp`; `gfx_sdl_set_backend` switches GL-context vs SDL_GPU window mode) |
| `gfx_sdlgpu.h` | SDL_GPU (Vulkan) rendering backend, opt-in via `Video.Renderer=sdlgpu` (implemented in `gfx_sdlgpu.cpp`; see `docs/PORT_SDLGPU.md`) |
