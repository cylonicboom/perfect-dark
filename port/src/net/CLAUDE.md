...
Fix (`chrTick`, right after `netChrInterpolate`, client-driven aibot sims only): drive `manground` from the **interpolated wire Y** before the anim tick. The host already applied gravity, so `prop->pos.y` carries the real fall trajectory — pinning `manground` to it each tick reproduces the host's smooth descent (a bot running off a ledge **floats down with the host's arc**, no snap) and still can't run away, because it's re-anchored to the authoritative, interpolated value every tick (no local integration). The chr's render root sits ~at `manground` (anim root translation ≈ 0 for locomotion), so `prop->pos.y` is the right value. Set `chr->ground = manground = simy`, `sumground = simy*scale`, `rwdata->chrinfo.ground = simy`, zero `fallspeed`; `cdFindGroundInfoAtCyl(&prop->pos,…)` is kept only as a **floor clamp** (`if (simy < floor) simy = floor`) so a landing undershoot can't dip below the real floor — and because the clamp only ever *raises*, a bad/low ground-find can't pull the bot down. `ground == manground` keeps `chr0f01f378` from adding any local fall.

  *History:* an earlier version pinned `manground` to the **ground found at `prop->pos`** (mirroring `chrSetPos`), which stopped the under-map sink but **snapped airborne bots to the floor** instead of letting them fall — the wire-Y version above replaces it. No wire/protocol change; `#ifndef PLATFORM_N64` + `g_NetMode == NETMODE_CLIENT` gated, so N64 / server / single-player are unaffected.
