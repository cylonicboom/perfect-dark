...
Fix (`chrTick`, right after `netChrInterpolate`, client-driven aibot sims only): re-assert `CHRCFLAG_FORCETOGROUND` every tick. `chr0f01f378` then snaps `manground` to the freshly-found ground at the synced X/Z (chr.c:881) instead of free-falling — the engine's own spawn-time grounding mechanism (chr.c:1151), reused per-tick. PD has no jump, so sims are only ever briefly airborne falling off a ledge; pinning them to the ground beneath (a step-down) beats vanishing. No wire/protocol change; `#ifndef PLATFORM_N64` + `g_NetMode == NETMODE_CLIENT` gated, so N64 / server / single-player are unaffected.
