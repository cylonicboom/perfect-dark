...
Fix (`chrTick`, right after `netChrInterpolate`, client-driven aibot sims only): re-pin the vertical **before** the anim tick to the ground found at the **synced `prop->pos`** — a per-tick mirror of what `chrSetPos` does on a net snap (chraction.c:16022): `cdFindGroundInfoAtCyl(&prop->pos, …)` → set `chr->ground`/`manground`/`sumground` + `rwdata->chrinfo.ground` + zero `fallspeed`. Each tick re-anchors `manground` to the real floor under the authoritative position, so the local integration can't run away. (`prop->pos` X/Z and the time-aligned rooms are what make the find land on the correct floor.) No wire/protocol change; `#ifndef PLATFORM_N64` + `g_NetMode == NETMODE_CLIENT` gated, so N64 / server / single-player are unaffected.
