...
- Lines that start with `/` are **local netplay/debug commands** routed through `netConsoleCommand` instead of being broadcast as chat. They work even outside a net session so you can pre-configure things like lag before connecting. Available commands:
  ...
  - `/netinfo` — print current net state (tick, mode, clients, sims, lag/loss settings, diag path).
  - `/help` / `/?` — list commands.
...
### Files Changed
- `port/include/net/net.h` — new constants, structs (`csp_snapshot`, `lagcomp_snapshot`, `netkillfeedentry`), ring buffer field in `netclient`, extern decls, function decls; `NET_PROTOCOL_VER 21`; `netplayermove.animnum` / `.animframe` for player anim sync; kill-feed buffer + render decls
...
- `port/src/console.c` — `/`-prefixed lines route to `netConsoleCommand` for local netplay/debug commands (e.g., `/lag`, `/loss`, `/diag`, `/netinfo`)
...
- CSP is back to the simpler retarget-only behavior. At very high ping the local player may drift slightly behind the authoritative position when constantly diverging from the server, but it's stable — no exponential teleporting.
