...
- `src/game/bondwalk.c` — rewrote `bwalkUpdateRemote` for ring-buffer snapshot interpolation; **early-return on server when `pl->ucmd & UCMD_FL_FORCEMASK`** — prevents `bwalkUpdateRemote` from writing the client's stale death-position `inmove` back into `prop->pos` on frames T+1+ after respawn. The sequence is: frame T runs `lvTickPlayer` (bwalkUpdateRemote, no force set yet) then `lvRender` (`playerStartNewLife`, sets `prop->pos=SPAWN_A` and force flags); frame T+1 runs `lvTickPlayer` (bwalkUpdateRemote sees `prop->pos=SPAWN_A` vs `inmove=DEATH_POS`, triggers the >512-unit snap, writes `DEATH_POS` back); `netEndFrame` then sends `SVC_PLAYER_MOVE(FORCE, DEATH_POS)` so the client `chrSetPos`s to the death point. The fix: return early when the server holds an authoritative force position, so `outmove[0].pos` stays `SPAWN_A` for all subsequent frames until acked. Works together with the `player.c` `playerStartNewLife` fix (client skips `scenarioChooseSpawnLocation` so it doesn't locally override the server's spawn either).
...
- `src/include/constants.h` — added `MPOPTION_NOCULL 0x10000000` (no portal culling), `MPOPTION_NOOMLIMIT 0x20000000` (no 60-room draw-slot cap); added `CHEAT_NOCULL 43`, `CHEAT_NODRAWLIMIT 44`, `CHEATFLAG_ALWAYSUNLOCKED 16`
- `src/game/bg.c` — `g_BgDrawSlots` expanded from 61 to 256 on port (sentinel moved to [255]); `g_BgSpecialDrawSlot` updated accordingly; `g_BgDrawSlots[60]` hardcoded refs replaced with `g_BgSpecialDrawSlot->`; `g_BgNoCull` / `g_BgNoDrawSlotLimit` port-only globals computed each frame in `bgTickPortals` from both MP option AND cheat active state; `bgSetRoomOnscreen` cap raised from 59 to 254 when `g_BgNoDrawSlotLimit`; local `roomorder`/`roomnums` arrays in `bgRenderScene` expanded to [255] on port; `#include "game/cheats.h"` added (port-only) for `cheatIsActive`
- `src/game/cheats.c` — `CHEAT_NOCULL` and `CHEAT_NODRAWLIMIT` added to `g_Cheats[]`; `CHEATFLAG_ALWAYSUNLOCKED` early-return in `cheatIsUnlocked`; `s_cheat_literal_names[]` port-only table for cheats without lang strings; `cheatGetName()` helper for marquee + name display; `cheatGetNameIfUnlocked` uses `cheatGetName`; `cheatGetMarquee` early-return for ALWAYSUNLOCKED cheats; `cheatActivate(CHEAT_NOCULL)` calls `gamefileUnlockEverything()` (unlocks all missions/cheats/weapons); `cheatActivate/Deactivate(CHEAT_NODRAWLIMIT)` sets `g_BgNoDrawSlotLimit`; both cheats added to `g_CheatsGameplayMenuItems`
- `src/game/mplayer/scenarios/combat.inc` — new "No Room Culling" and "No Draw Slot Limit" checkboxes (port-only) added after "Controllers Only" inside `#ifndef PLATFORM_N64`
- `port/src/net/netmenu.c` — `No Room Culling` and `No Draw Limit` added to the lobby active-options summary string
