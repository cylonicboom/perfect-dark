...
- `Silo.mp3` — used by the "Silo Countdown" effect (the self-destruct track,
  played looped with the mission music killed underneath). The final-stretch
  music is baked into this single track now, so there is no separate final-stretch
  file. Best-effort: the countdown and detonation still run if the file is
  missing (just silent). Not shipped with the repo (copyrighted audio).
