...
#define NET_PROTOCOL_VER 89 // 89: BOTDIFF_DEMON (7) — the port-only "Demon" simulant difficulty (docs/PORT_DEMON_SIMS.md). No layout change: bot difficulty was already a u8 in SVC_STAGE_START / CLC_ADMIN_SETUP and a 3-bit field on disk. Bumped because the VALUE 7 is new and an old peer receiving it indexes g_BotDifficulties[7] — one past its 7-row table — producing an out-of-bounds read that drives that sim's aim/reaction/dizzy thresholds off garbage. New builds sanitize the field on read (netmsgSanitizeBotDifficulty) but old ones cannot, so mixed versions must not join. 88: three wire-width fixes found in the branch audit. (a) SVC_ELIM_STATE's eliminated set was a u16 bitmask over MAX_MPCHRS (48) slots — simulants wire-key at MAX_PLAYERS+ordinal (16..47), so every sim's eliminated flag truncated away (host said "out", clients never agreed, so lives HUD / elimChrCanRespawn / match-end disagreed permanently) and `1u << i` for i >= 32 was undefined behaviour on both sides; it is now one byte per combatant, appended after the team-pool block. (b) prop->syncid is u32 and g_NetNextSyncId free-runs for a whole stage, but CLC_PICKUP_REQUEST, CLC_DOOR_ACTIVATE and SVC_CARRY_STATE marshalled it as u16 — past 65535 the server resolved a DIFFERENT prop (70000 -> 4464) and ran objTestForPickup / propdoorInteract on it as the requesting client; those three are now u32 (SVC_PROP_RECONCILE keeps its documented, guarded u16 cap). The server-side hit-validation ring srvhits[].syncid was u16 for the same reason and is now u32. (c) netvotecandidate.playlist_index was s8 against a 256-entry playlist, so entry 130 wrapped to -56 and entry 255 to exactly -1 — the RANDOM sentinel — making entries past 127 unvotable; now s16 (0xFFFF sentinel on the wire). Old peers mis-parse all three, so mixed versions must not join. 87: (a) client->host weapon heartbeat — new unconditional s8 curweaponnum on netplayermove (the client's CURRENTLY-equipped weapon, sent every move, change-detected before animnum), reconciled on the host in bmoveProcessRemoteInput so a remote co-op/Combat-Sim player's third-person gun + firing are correct even when the initial/scripted loadout or a UCMD_SELECT edge was never seen (old switch-edge-only sync left the host on a stale/absent weapon -> wrong gun / no gun / no muzzle flash). (b) new u8 g_MpChallengeNumPlayers appended to SVC_STAGE_START (Combat block) and CLC_ADMIN_SETUP — Combat Sim challenge difficulty override (0=auto, 1..4 forced player count) so online co-op challenges can be made harder regardless of the connected player count. Old peers can't parse the extra move field / option byte, so mixed versions must not join. 86: "wire owns the lifetime" for chr-held drops — the server spawn-broadcasts at the objSetDropped chokepoint (corpse drops, scripted disarms, owner reaps) and clients suppress their local corpse-drop twins (chrBeginDeath). Mixed versions would disagree on who owns the floor gun (old server never broadcasts at drop -> new client shows no gun in co-op), so they must not join. 85: SVC_ALARM (0x5d) — co-op host-authoritative alarm mirror (on change + 1s heartbeat at phase 25). Also (no wire change, same bump): chrResolveId now remaps CHR_BOND/CHR_COOP to the transposed local slot on co-op clients (the generalised "option 3" — every player-targeted AI command now hits the same physical player as the host: gives, control grants/revokes, hudmsgs/subtitles, disarms, draw-weapon, fades, autowalk, aiIf* conditionals); mixed versions would run different script targeting, so they must not join. 84: MPOPTION_AUTOLIFTS (options bit 51) — "Auto Lifts" Combat Sim option disables manual lift calling (doorCallLift) and keeps lifts cycling on their own (liftTick clears OBJFLAG_DEACTIVATED/LIFT_TRIGGERDISABLE). Rides g_MpSetup.options (already wire-synced); bumped because both peers must clear the lift deactivation identically or a deactivated lift desyncs (server cycles it, an old client leaves it stuck). 83: SVC_CTC_CAPTURE (0x5c) — Capture-the-Case scoring event. The scoring branch in scenarioPickUpBriefcase returns TICKOP_NONE, so propPickupByPlayer's SVC_PROP_PICKUP broadcast never fired and clients got no capture SFX/HUD (only the score via SVC_SCORE). New one-shot event carries the capturer (wire-keyed) + captured team; clients play SFX_MP_SCOREPOINT + the 3-way "captured" HUD and clear the local capturer's briefcase (so the re-collection's pickup branch + sound/HUD work again). carryApplyWireState also restores the re-spawned ground case's weapon->team on clients (SVC_PROP_SPAWN omits it) so the pickup replay routes correctly + the case highlight colour is right. 82: SVC_SCORE killcounts column fix — the per-victim killcounts sub-array was indexed by the PACKED g_MpAllChrConfigPtrs index, but mpchr->killcounts[] is SLOT-indexed (players 0..MAX_PLAYERS-1, bots MAX_PLAYERS+; func0f18d074/MPCHR/scenarioCalculatePlayerScore). For humans packed==slot so individual scores were fine, but a bot's packed index != its slot, so every human-vs-bot kill was read/written to the wrong column → team scores (which sum killcounts via slot indices) read 0. Now converts the packed victim index to its slot via func0f18d074 on both write and read. Wire byte layout unchanged; bumped because the bot-column interpretation differs across builds. 81: MPOPTION_LASTATTACKERKILL (options bit 50) — "Last Attacker Attribution" Combat Sim option credits env/fall/knockback/suicide-play deaths to chr->lastattacker. Rides g_MpSetup.options (already wire-synced); bumped because the kill-attribution gate semantics differ across builds. 80: per-combatant wire-keying fixed for SIMULANTS — bots are packed below MAX_PLAYERS in g_MpAllChrPtrs, so the old "idx<MAX_PLAYERS?netid:raw" scheme mis-keyed every bot to 0xff and the reader dropped it (sim scores/kills/lives/race-progress never synced). Bots now key by MAX_PLAYERS+ordinal (netMpchrToWireKey). Affects SVC_SCORE, SVC_ELIM_STATE, SVC_RACE_STATE, SVC_CARRY_STATE, SVC_HTM_STATE, SVC_PAC_STATE. Human keying unchanged. 79: SVC_CARRY_STATE now carries the on-ground case syncid per token so CTC dropped/returned cases resolve by syncid (SVC_PROP_SPAWN doesn't carry weapon->team, and the team is set after the spawn broadcast). 78: carry-object scenarios made server-authoritative + synced — new SVC_CARRY_STATE (0x59: HTB/CTC per-token holder, wire-keyed, + CTC home team), SVC_HTM_STATE (0x5a: Hack-that-Mac uplink holder + active downloader + terminal team + progress), SVC_PAC_STATE (0x5b: Pop-a-Cap current victim). Clients no longer create local token/uplink ghosts (lifecycle is server-only); holder/victim state arrives on the wire. Mixed versions must not join.
...
// Combat Sim challenge difficulty override (port-only, session-only, synced via
// SVC_STAGE_START). 0 = auto (scale by the real player count, vanilla). 1..4 =
// force that many "players" for challenge scaling so an online co-op challenge
// can be made harder regardless of how many humans are actually connected —
// feeds both mpCalculateTeamScoreLimit (score target) and challengePerformSanity-
// Checks (simulant count + difficulty table). Host setting; clients receive it.
extern s32 g_MpChallengeNumPlayers;
...
// Team-byte sentinels. 0xff = unknown team → render falls back to the
// local-red / remote-green palette by name-match (online). 0xfe = "this is a
// local human player" → always red (offline, where there's no g_NetLocalClient
// to match names against). Real teams are 0..7. 0xfe is never put on the wire
// (offline has no SVC_KILL), so no protocol impact.
#define NET_KILLFEED_TEAM_NONE      0xff
#define NET_KILLFEED_TEAM_LOCAL     0xfe
...
struct netplayermove {
	...
	s8 curweaponnum; // the weapon the client currently holds in the right hand (proto 87); sent every move so the host reconciles a remote pawn's gun even when a UCMD_SELECT edge / scripted loadout was never seen. Before animnum => change-detected by netClientNeedMove.
...
struct netclient {
	...
	// syncid is u32: a u16 here aliased a shot at prop 70000 onto prop 4464
	// once the stage counter passed 65535, validating hits against the wrong
	// prop.
	struct { u32 syncid; u32 tick; } srvhits[NET_SRVHIT_COUNT];
	u32 srvhits_head;
...
// True when g_Vars.currentplayer is the PRIMARY LOCAL viewport: the
// netplay-safe form of the raw `currentplayernum == 0` idiom for sites that
// mean "the one local player" (once-per-frame HUD state advances, local-only
// cutscene gates). Offline/splitscreen: viewport 0 verbatim. Do NOT use for
// splitscreen viewport-layout math — those `== 0` gates key on the viewport
...
struct netvotecandidate {
	// s16: PLAYLIST_MAX_ENTRIES is 256, so an s8 wrapped entry 130 to -56 and
	// entry 255 to exactly -1 — the RANDOM sentinel. Entries past 127 could
	// never be voted for; the ballot slot rendered as "Random" and the vote
	// resolved to a freshly-picked entry instead.
	s16 playlist_index;     // -1 (0xFFFF on wire) for the RANDOM slot
	u8  stagenum;
	u8  scenario;
...
// Wipe the kill feed. Called on net session teardown and at every match start
// (mpStartMatch) so stale entries don't carry into the next match — important
...
// `eliminated` is one byte per combatant, NOT a bitmask: bots wire-key at
// MAX_PLAYERS+ordinal (16..47), so the old u16 mask dropped every simulant
// and shifted past the type width for slots >= 32 (UB).
void elimApplyWireState(const u8 *lives, const u8 *teamlives, const u8 *eliminated);
...
// Client apply for SVC_CTC_CAPTURE: replay the capture (score) SFX + 3-way HUD and
// clear the local capturer's briefcase so the next collection routes correctly.
