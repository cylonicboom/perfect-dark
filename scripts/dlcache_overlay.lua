...
-- record, `draw` (batches/tris replayed) rise and fall as you move/cull, and the
-- `bad:` reason flags tell you why some leaves stayed on the legacy path
...
pd.on("draw", function()
  ...
  if s.bad > 0 then
    ...
    if r == "" then r = " ?" end
    pd.draw_text(X, Y + 26, "bad:" .. r, C_YELL)
  end
end)
