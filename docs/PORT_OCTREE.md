...
| `/octree auto` (alias `outdoor`) | toggle `g_BgOctreeAutoOutdoor` — like `markall` but **filtered to outdoor rooms only** (`room->flags & ROOMFLAG_OUTDOORS`, set from level data). Octree-culls every outdoor room with **no manual `/octree mark`**, building each lazily on first sight. This is the "wired to level data" path: outdoor rooms (where the octree pays off) auto-engage, indoor rooms keep the cheap unculled path. Forces the master switch on. Default **off** (so the build stays byte-identical until you opt in); flip the `g_BgOctreeAutoOutdoor` initialiser in `bg.c` to make it the default once validated per-level. No permanent `extra_flags` mutation, so toggling is instant. |
