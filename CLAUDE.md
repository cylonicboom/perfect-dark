...
- `docs/PORT_NODOORS.md` — Port-only "No Doors" Combat Sim option. **Introduces the `g_MpSetup.portoptions` overflow word** — the first MP option that didn't fit the now-full 32-bit `options` field. Read when adding any MP option that overflows `options`, or when touching `setupMarkLiftDoors`/the `OBJTYPE_DOOR` skip in `setup.c`, `menuhandlerMpCheckboxPortOption`, or the `portoptions` bytes in `SVC_STAGE_START` / `CLC_ADMIN_SETUP`. Note `MPOPTION_NODOORS 0x00000001` aliases `MPOPTION_ONEHITKILLS`'s bit — they're distinct only by field.
