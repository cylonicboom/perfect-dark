...
| `/dlcache indexed` | toggle the **indexed upload** (A5 round-2 opt) for newly recorded entries: staged vertices are deduped by full bit-equality per same-program segment run and drawn via a u32 index buffer (`cache_draw_indexed`), typically ~2-3× fewer vertices shaded. Entries remember their layout; `/dlcache clear` re-records everything. Default on. Stats line shows the dedup ratio. Backends without the index entries (GL ES < 3.0) silently stay non-indexed. |
| `/dlcache packed` | toggle the **packed cached vertex layout** (A5 half 2) for newly recorded entries: the fog / grayscale / combiner-input attributes are stored as one normalized u8x4 slot each instead of 3-4 floats (fed to the unchanged float shader inputs via `GL_UNSIGNED_BYTE`+normalize / `UBYTE4_NORM` — **lossless**, every packed source is u8-derived). Typical room vertex shrinks 60 → 40 bytes on top of the indexed dedup. Per-entry latch like `indexed`; the replay tells the backend which layout via `cache_set_packed`, and on SDL_GPU packed cached draws use their own pipeline-key bit (15). **Escape hatch if a driver misreads packed attribute formats** (wrong colours/fog ONLY with `/dlcache on`): flip this + `/dlcache clear`. Default on. |
