	...
	if (race != RACE_DRCAROLL && race != RACE_ROBOT) {
		...
		if (chr->actiontype == ACT_DEAD || chr->actiontype == ACT_DIE) {
			...
			if (thudframe != -1.0f && modelGetCurAnimFrame(chr->model) < thudframe) {
				...
				chr->deaddropsplatsadded += splatsCreate(1, 1.1f, prop, NULL, 0, 0, isskedar, SPLATTYPE_PUDDLE, TICKS(150), attacker, rngCosmeticRandom() & 8);
			}
		} else {
...
void splat0f14986c(struct splatdata *splat)
{
	...
	texnum = WALLHITTEX_BLOOD1 + (rngCosmeticRandom() % 3);
	...
	case SPLATTYPE_PUDDLE:
		...
		break;
	case SPLATTYPE_DROP:
		texnum = WALLHITTEX_BLOOD4 + (rngCosmeticRandom() % 1);
		break;
	}

	switch (rngCosmeticRandom() % 6) {
	case 0:
	case 1:
	case 2:
		sp9c = 1.5f;
	...
	wallhitCreateWith20Args(&splat->relpos, &splat->unk18, splat->chr ? &splat->chr->prop->pos : NULL, NULL,
			...
			rngCosmeticRandom() % 360, (u16)splat->timermax, splat->timerspeed, translucent);
